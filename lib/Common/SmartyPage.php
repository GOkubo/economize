...
class SmartyPage extends Smarty
{
	...
	public function DisplayLocalized($templateName, $languageCode = null)
	{
		...
		$defaultPath = ROOT_DIR . 'lang/pt_br/' . $templateName;
	...
	public function FetchLocalized($templateName, $languageCode = null)
	{
		...
		$localizedPath = ROOT_DIR . 'lang/' . $languageCode;
		$defaultPath = ROOT_DIR . 'lang/pt_br/';
	...
	protected function RegisterFunctions()
	{
		...
		$this->registerPlugin('function', 'html_imageSub', array($this, 'PrintImageSub'));
		...
		$this->registerPlugin('function', 'jsfileSub', array($this, 'IncludeJavascriptFileSub'));
		$this->registerPlugin('function', 'cssfileSub', array($this, 'IncludeCssFileSub'));
	}
	...
	public function PrintImage($params, &$smarty)
	{
		...
		$imgPath = sprintf('%s../images/%s', $this->RootPath, $params['src']);
        ...
        public function PrintImageSub($params, &$smarty)
	{
		$alt = isset($params['alt']) ? $params['alt'] : '';
		$altKey = isset($params['altKey']) ? $params['altKey'] : '';
		$width = isset($params['width']) ? $params['width'] : '';
		$height = isset($params['height']) ? $params['height'] : '';
		$imgPath = sprintf('%simages/%s', $this->RootPath, $params['src']);
...
//                echo '<br>';print_r("src= " . $this->RootPath . "../JavaScripts/" . $params['src'] . "?v=" . $versionNumber . " type=\"text/javascript\"></script>");
		echo "<script src=\"{$this->RootPath}../{$params['src']}?v=$versionNumber\" type=\"text/javascript\"></script>";
	}
	...
	public function IncludeJavascriptFileSub($params, &$smarty)
	{
		$versionNumber = Configuration::VERSION;
//                echo '<br>';print_r("src= " . $this->RootPath . "../JavaScripts/" . $params['src'] . "?v=" . $versionNumber . " type=\"text/javascript\"></script>");
		echo "<script src=\"{$this->RootPath}../../{$params['src']}?v=$versionNumber\" type=\"text/javascript\"></script>";
	}
	public function IncludeCssFileSub($params, &$smarty)
	{
		$versionNumber = Configuration::VERSION;
		$src = $params['src'];
		if (!BookedStringHelper::Contains($src, '/'))
